...
class BaseDataset(Dataset):
    ...
    Attributes:
        ...
        ims_rgb (list): List of loaded images.
        ims_depth (list): List of loaded depth images.
        npy_files_rgb (list): List of numpy file paths.
        npy_files_depth (list): List of numpy depth file paths.
    ...
    def load_image(self, i, rect_mode=True):
        ...
        if im_rgb is None:  # not cached in RAM
            if fn_rgb.exists():  # load npy
                try:
                    im_rgb = np.load(fn_rgb)
                    im_depth = np.load(fn_depth)
                except Exception as e:
                    LOGGER.warning(f"{self.prefix}WARNING ⚠️ Removing corrupt *.npy image file {fn_rgb} due to: {e}")
                    Path(fn_rgb).unlink(missing_ok=True)
                    Path(fn_depth).unlink(missing_ok=True)
                    im_rgb = cv2.imread(f_rgb)  # BGR
                    im_depth = cv2.imread(f_depth)  # depth
            else:  # read image
                im_rgb = cv2.imread(f_rgb)  # BGR
