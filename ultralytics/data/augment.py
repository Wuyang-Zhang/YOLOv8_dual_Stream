...
class RandomPerspective:
    ...
    def affine_transform(self, img_rgb, img_depth, border):
        ...
        Args:
            img_rgb (ndarray): Input image.
            img_depth (ndarray): Input depth image.
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            im_hsv_depth = cv2.merge((cv2.LUT(hue_depth, lut_hue), cv2.LUT(sat_depth, lut_sat), cv2.LUT(val_depth, lut_val)))
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if labels is None:
            labels = {}
        # img_rgb = labels.get('img_rgb') if image[0] is None else image[0]
        # img_depth = labels.get('img_depth') if image[1] is None else image[1]
        if image is not None:
            if isinstance(image, np.ndarray):
                img_rgb = image
                img_depth = image
            elif image[0] is None:
                img_rgb = labels.get('img_rgb')
                img_depth = labels.get('img_depth')
            else:
                img_rgb = image[0]
                img_depth = image[1]
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                cv2.drawContours(im_new_rgb, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
            ...
            i_rgb = cv2.flip(im_new_rgb, 1).astype(bool)
            im_rgb[i_rgb] = result_rgb[i_rgb]
